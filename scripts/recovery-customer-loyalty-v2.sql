-- ─── Recovery v2 — minimal customer_summary infrastructure ─────────────────
--
-- Confirmed from production schema probe:
--   • children.full_name  (NOT children.name)
--   • parents has: id, full_name, phone, wallet_balance, created_at, branch_id
--   • sessions has: parent_id, status, created_at
--   • payments has: session_id, total_amount, created_at
--
-- This recovery deliberately AVOIDS wallet_transactions and refund_logs
-- joins to stay simple and not block on those tables' column shapes.
-- It can be extended in a follow-up.

create extension if not exists pg_trgm;

-- 1. parents extensions (idempotent)
...
create index if not exists idx_children_full_name_trgm on public.children using gin (full_name gin_trgm_ops);

-- 2. backfill last_visit_at from sessions
...
-- 3. trigger to keep last_visit_at fresh on new sessions
...
-- 4. customer_summary view (MINIMAL — sessions + payments + children)
...
-- 5. customer_activity view (MINIMAL — sessions + payments only)
drop view if exists public.customer_activity cascade;
create view public.customer_activity as
select
...
-- 6. search_customers RPC — phone / parent name / child full_name
...
set search_path = public
as $$
  with q as (select trim(coalesce(p_query, '')) as v)
  select * from public.customer_summary cs
   where case
           when (select length(v) from q) < 2 then false
           else
             cs.full_name ilike '%' || (select v from q) || '%'
             or cs.phone ilike '%' || (select v from q) || '%'
             or exists (
               select 1 from public.children c
                where c.parent_id = cs.id
                  and c.full_name ilike '%' || (select v from q) || '%'
             )
         end
...
begin
  ...
  select coalesce(json_agg(c.* order by c.created_at), '[]'::json)
    into v_children
    from (
      select id, full_name as name, age, allergies, created_at
        from public.children
...
-- 9. set_customer_tag RPC (manager/admin only)
...
-- ── Verify ───────────────────────────────────────────────────────────────
select 'customer_summary rows' as label, count(*)::text as value from public.customer_summary
union all
select 'customer_activity rows',          count(*)::text from public.customer_activity
union all
select 'parents total',                   count(*)::text from public.parents
union all
select 'sessions total',                  count(*)::text from public.sessions
union all
select 'payments total',                  count(*)::text from public.payments;
