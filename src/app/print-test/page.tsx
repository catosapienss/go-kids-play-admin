...
import { Printer, User, Users, Loader2, Eye, Hash, Phone, AlertCircle } from "lucide-react"
...
// ─── /print-test — Sample Label Printer ──────────────────────────────────────
//
...
// printLabels() helper that the post-registration success modal uses.

const DEFAULT_CHILD: ChildLabelData = {
  childName:     "Defne Yılmaz",
  startTime:     "14:30",
  endTime:       "15:00",
  durationLabel: "30 dk",
...
const DEFAULT_PARENT: ParentLabelData = {
  childName:     "Defne Yılmaz",
...
export default function PrintTestPage() {
  ...
  const [parent, setParent] = useState<ParentLabelData>(DEFAULT_PARENT)
  ...
  // Inline previews use the same render functions the printer does.
  ...
  const previewCss = `
    ...
    .preview .row { display:flex; align-items:baseline; justify-content:space-between; gap:4mm; }
    .preview .name { font-size:${Math.min(printer.labelWidthMm, printer.labelHeightMm) >= 40 ? "7mm" : "5mm"};
                     font-weight:900; line-height:1.05; text-transform:uppercase; letter-spacing:0.02em; word-break:break-word; }
    .preview .meta { font-size:3mm; font-weight:700; text-transform:uppercase; letter-spacing:0.06em; opacity:0.7; }
    .preview .strong { font-size:4.5mm; font-weight:800; }
    .preview .session { font-family:ui-monospace,'SF Mono',Menlo,monospace; font-size:3.5mm; font-weight:900;
                        letter-spacing:0.1em; padding:1mm 2mm; border:0.4mm solid #000; border-radius:1mm; }
    .preview .row-2 { font-size:4mm; font-weight:700; }
    .preview .phone { font-size:5mm; font-weight:900; letter-spacing:0.04em; }
  `

  return (
    <MainLayout title="Etiket Test" subtitle="Operasyonel veriye dokunmadan XPrinter XP-470B testi">
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              <LabeledInput label="Süre" value={child.durationLabel}
                onChange={(v) => setChild({ ...child, durationLabel: v })} placeholder="30 dk" />
              <LabeledInput label="Oturum #" value={child.sessionNumber}
                onChange={(v) => setChild({ ...child, sessionNumber: v })} icon={<Hash className="w-3.5 h-3.5"/>} />
            </div>
          </Panel>
          ...
          >
            ...
            <LabeledInput label="Veli adı" value={parent.parentName}
              onChange={(v) => setParent({ ...parent, parentName: v })} />
            <LabeledInput label="Telefon" value={parent.parentPhone}
              onChange={(v) => setParent({ ...parent, parentPhone: v })} icon={<Phone className="w-3.5 h-3.5"/>} />
            <LabeledInput label="Oturum #" value={parent.sessionNumber}
              onChange={(v) => setParent({ ...parent, sessionNumber: v })} icon={<Hash className="w-3.5 h-3.5"/>} />
          </Panel>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Otomatik yazdırma:&nbsp;
                <strong>{printer.autoPrintEnabled ? "Açık" : "Kapalı"}</strong>
              </p>
            </div>
            <button
              type="button"
