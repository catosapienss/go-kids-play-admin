...
const DEMO_ACCOUNTS = [
  { role: "Admin", email: "admin@gokids.com", password: "demo1234" },
  { role: "Yönetici", email: "yonetici@gokids.com", password: "demo1234" },
  { role: "Kasiyer", email: "kasiyer@gokids.com", password: "demo1234" },
]
...
export default function LoginPage() {
  ...
  useEffect(() => {
    setMounted(true)
    const saved = localStorage.getItem("gkp_remember_email")
    if (saved) {
      setEmail(saved)
      setRemember(true)
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (remember) {
        localStorage.setItem("gkp_remember_email", email)
      } else {
        localStorage.removeItem("gkp_remember_email")
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-slate-950">
      {/* Ambient gradient blobs — operational violet kept, but a sky-green wash
          added on the left to echo the brand without overwhelming the form. */}
      ...
      >
        ...
        <div className="relative bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-3xl shadow-2xl shadow-black/40 overflow-hidden">
          {/* Brand accent strip — subtle green/pink/yellow band along the top edge */}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                autoComplete="email"
                disabled={loading}
                className="w-full px-4 py-3 rounded-2xl bg-white/[0.06] border border-white/[0.1] text-white placeholder:text-white/25 text-sm outline-none focus:border-violet-500/60 focus:bg-white/[0.08] focus:ring-2 focus:ring-violet-500/20 transition-all disabled:opacity-50"
              />
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-white/[0.07]">
            <p className="text-[11px] font-semibold text-white/25 uppercase tracking-wider text-center mb-3">
              Demo Hesapları
            </p>
            <div className="grid grid-cols-3 gap-2">
              {DEMO_ACCOUNTS.map((acc) => (
                <button
                  ...
                  className="flex flex-col items-center gap-1 py-2.5 px-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] hover:border-white/[0.12] transition-all group"
                >
        ...
        <p className="text-center text-[11px] text-white/25 mt-5">
          ...
          <span className="opacity-50"> · Yönetim Sistemi v2.1 · © 2026</span>
        </p>
      </div>
    </div>
  )
