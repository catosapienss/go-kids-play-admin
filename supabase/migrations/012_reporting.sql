-- ============================================================
-- GoKids Play — Advanced Reporting & Business Insights
-- Migration 012
--
-- Adds the high-leverage aggregate RPCs that power the /raporlar dashboard.
-- All RPCs:
--   • Accept an optional date range (`p_from`, `p_to` timestamptz) so the
--     UI's date filter pushes filtering all the way down to the planner.
--   • Are branch-aware via the existing RLS pattern (is_super_admin() or
--     branch_id = current_branch()).
--   • Return jsonb / row-set shapes the client can render in one render pass —
--     no waterfall hydration needed.
--
-- For multi-branch comparison later, swap the single branch predicate for a
-- group-by branch_id in the call sites — no schema changes needed.
-- ============================================================

-- ─── Helper: resolve effective date window ───────────────────────────────────
--
-- p_from/p_to are both nullable. NULLs default to "today" for closed ranges
-- so plain `select get_revenue_periods()` still returns sensible data.

create or replace function public._effective_range(
  p_from timestamptz,
  p_to   timestamptz
) returns table(d_from timestamptz, d_to timestamptz)
language sql
immutable
as $$
  select
    coalesce(p_from, date_trunc('day', now()))                   as d_from,
    coalesce(p_to,   date_trunc('day', now()) + interval '1 day') as d_to;
$$;

-- ─── 1. Revenue periods card (today / 7d / 30d / 365d + prior period delta) ──
...
declare
  ...
  v_yesterday      timestamptz := v_today_start - interval '1 day';
  v_week_start     timestamptz := v_today_start - interval '7 days';
  v_prev_week      timestamptz := v_today_start - interval '14 days';
  v_month_start    timestamptz := v_today_start - interval '30 days';
  v_prev_month     timestamptz := v_today_start - interval '60 days';
  v_year_start     timestamptz := v_today_start - interval '365 days';
begin
  return jsonb_build_object(
    'today',     period_total(v_today_start,  v_today_start + interval '1 day'),
    'yesterday', period_total(v_yesterday,    v_today_start),
    'week',      period_total(v_week_start,   v_today_start + interval '1 day'),
    'prev_week', period_total(v_prev_week,    v_week_start),
    'month',     period_total(v_month_start,  v_today_start + interval '1 day'),
    'prev_month',period_total(v_prev_month,   v_month_start),
    'year',      period_total(v_year_start,   v_today_start + interval '1 day')
  );
end;
$$;

-- Helper that powers get_revenue_periods.
...
begin
  ...
  select count(*) into v_total_customers
    from public.parents
    where (public.is_super_admin() or branch_id = public.current_branch());
  ...
  select round(coalesce(avg(spend), 0), 2), coalesce(sum(visits), 0)::integer
    ...
    from (
      ...
      where sess.parent_id is not null
        ...
        and (public.is_super_admin() or sess.branch_id = public.current_branch())
...
as $$
  ...
  shifts_by_staff as (
    ...
    where s.started_at >= (select d_from from bounds)
      ...
      and (public.is_super_admin() or s.branch_id = public.current_branch())
