...
as $$
  ...
  firstret as (
    ...
    select
      ...
      count(*) filter (where first_ever <  (select d_from from r))::int returning
    from (
  ...
  select jsonb_build_object(
    ...
    'returning_children',  (select returning from firstret),
...
as $$
  ...
  entries as (
    select (s.created_at at time zone 'Europe/Istanbul')::date day, count(*) c
    from public.sessions s, r
    where s.created_at >= r.d_from and s.created_at < r.d_to
      and (public.is_super_admin() or public.current_branch() is null
           or s.branch_id is null or s.branch_id = public.current_branch())
  ...
  play as (
    select (p.created_at at time zone 'Europe/Istanbul')::date day, sum(p.total_amount) amt
    from public.payments p, r
    where p.created_at >= r.d_from and p.created_at < r.d_to and p.session_id is not null
    group by 1
  ),
  retail as (
    select (rs.sold_at at time zone 'Europe/Istanbul')::date day, sum(rs.total_amount) amt
    from public.retail_sales rs, r
    where rs.sold_at >= r.d_from and rs.sold_at < r.d_to and coalesce(rs.voided,false) = false
    group by 1
  )
  select d.day, coalesce(e.c,0)::int, coalesce(pl.amt,0)::numeric, coalesce(rt.amt,0)::numeric
  from days d
  left join entries e on e.day = d.day
  left join play    pl on pl.day = d.day
  left join retail  rt on rt.day = d.day
  order by d.day;
$$;
